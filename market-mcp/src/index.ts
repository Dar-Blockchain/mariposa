...
class GeckoTerminalAPI {
  ...
  async fetchSailorDexPools(networkId: string = 'sei-evm'): Promise<Pool[]> {
    console.log(`🚢 Fetching all Sailor DEX pools from ${networkId}...`);
    return this.fetchAllPoolsByDex(networkId, 'sailor');
  }

  async fetchSeiEvmAllDexPools(networkId: string = 'sei-evm'): Promise<Pool[]> {
    console.log(`🌊 Fetching pools from multiple DEXes on ${networkId}...`);
    
    const targetDexes = ['sailor', 'dragonswap', 'yaka-finance', 'dragonswap-v2'];
    const allPools: Pool[] = [];
...
class MarketMCPServer {
          ...
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                network: {
                  type: 'string',
                  description: 'Network ID (e.g., sei-evm, eth, bsc, polygon)',
                },
                hours_back: {
                  type: 'number',
  ...
  private async handleGetNetworkPools(args: any) {
    ...
    try {
      ...
      // For SEI-EVM, fetch from multiple DEXes to get diverse tokens
      if (network === 'sei-evm') {
        console.log(`🌊 [MCP HANDLER] [${requestId}] Using specialized SEI-EVM multi-DEX strategy`);
        if (page === 1) {
          console.log(`🌊 [MCP HANDLER] [${requestId}] Page 1: Fetching from multiple DEXes`);
          pools = await this.api.fetchSeiEvmAllDexPools(network);
        } else {
          console.log(`🌊 [MCP HANDLER] [${requestId}] Page ${page}: Using Sailor DEX pagination`);
          pools = await this.api.fetchPoolsByDex(network, 'sailor', page);
        }
      } else {
  ...
  private async handleGetAllTokens(args: any) {
    ...
    const startTime = Date.now();
    const { network = 'sei-evm', includeScoring = true } = args;
    ...
    try {
      ...
      const pools = await this.api.fetchSeiEvmAllDexPools(network);
